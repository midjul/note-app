...
if (Meteor.isServer) {
    describe('notes', function () {
        ...
        it('should throw error if extra updates', function () {
            expect(() => {
                Meteor.server.method_handlers['notes.update'].apply({ userId: noteOne.userId }, [{ newProp: "newProp" }])
            }).toThrow();
        });
        ...
        it('should return no notes for user that has none', function(){
            const res = Meteor.server.publish_handlers.notes.apply({ userId: "testingId"})
            const notes=res.fetch();
